...
def grid_learn_method(base_name):
    ...
    res, best, rand = RefaelLearner().run_al(rand=True)
    ...
    res, best, _ = RefaelLearner().run_al()
    pickle._dump(res, open(os.path.join(base, "res_NN_learn_method"), "wb"))
    REFAEL_PARAM['learn_method'] = "XGBoost"
...
def grid_min_nodes(base_name, min_nodes_list: list):
    min_nodes_to_batch_needed = {0: 24, 3: 15, 5: 8, 10: 7}
...
if __name__ == "__main__":
    ...
    base_name_ = "241218_161454"
    # create_res_dir(base_name_)

    # grid_slide_window(base_name_, [1, 5, 10, 15])
    # grid_batch_size(base_name_, [(1, 15), (3, 5), (5, 3), (15, 1)])
    # grid_eps(base_name_, [0.01, 0.1,  0.3])
